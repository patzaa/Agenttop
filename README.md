...
The actual background agents running on this machine (upgrade validation, release-notes review, security/UI watch, deploy validation, audits, worktree cleanup) live in [`patzaa/autonomous-background-agents`](https://github.com/patzaa/autonomous-background-agents) — scripts, launchd plists, shared libraries, and the clickable-notification helpers, with a README describing every agent and its schedule.
